...
export function DriverRow({ driver, gapMode }: DriverRowProps) {
  const teamColor = getTeamColorFromHex(driver.team_colour);
  const gap = gapMode === 'leader' ? driver.gap_to_leader : driver.interval;

  return (
    <div
      className="driver-row grid grid-cols-[3rem_1fr_5rem_5.5rem_4.5rem_4.5rem_4.5rem_3rem_2.5rem] gap-0 px-3 py-1.5 items-center border-b border-f1-border/50 hover:bg-f1-elevated/30 text-sm"
